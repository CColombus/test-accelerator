...
class TestAcceleratorModule(outer: TestAccelerator)(implicit p: Parameters)
  ...
  object FSMstate extends ChiselEnum {
    val IDLE, LOAD_REQ, LOAD_SETUP, LOAD_RESP, CALC, STORE_REQ, STORE_RESP = Value
  }
  ...
  val loadIdx   = RegInit(0.asUInt(log2Ceil(4).W)) // index for the load vector
  ...
  // latch register for memory addresses
  val regLoadPtr  = Reg(UInt(32.W)) // register to hold the load pointer
  val regStorePtr = Reg(UInt(32.W)) // register to hold the store pointer

  // variables used to latch memory requests
  val reqAddr = Reg(UInt(32.W))
  val reqTag  = RegInit(0.asUInt(log2Ceil(4).W)) // assuming 4 tags for the vector
  ...
  val reqSetupComplete = RegInit(false.B) // indicates if the request setup is complete
  ...
  switch(state) {
    is(IDLE) {
      when(cmd.valid && doAcc && !memReqValid) {
        ...
        reqSetupComplete := false.B // reset the request setup state

        regLoadPtr  := loadPtr  // latch the load pointer
        regStorePtr := storePtr // latch the store pointer

        reqAddr := loadPtr // use load pointer as the address
    ...
    is(LOAD_SETUP){
      // This state is used to set up the load request
      // It can be used to prepare the address and tag for the load request
      when(!reqSetupComplete) {
        reqAddr := regLoadPtr// use the load pointer as the address
        // reqData := 0.asUInt(32.W)        // no data to send yet
        // reqCmd  := M_XRD      // set the command to read
        reqSetupComplete := true.B // mark the request setup as complete

      }.otherwise {
        printf(cf"*ta*LOAD_SETUP~Request setup complete for tag $reqTag at address $reqAddr.\n")
        state := LOAD_REQ // transition to LOAD_REQ state
      }
    }
    is(LOAD_REQ) {
    ...
    is(CALC) {
      ...
      // TODO: for now bypass to IDLE
      accDone := true.B // mark the accumulation as done
      state   := IDLE   // bypass to IDLE for now
    }
    is(STORE_REQ) {
      when(io.mem.req.fire) {
...
// TODO: Change from here onwards
  ...
  io.mem.req.bits.cmd     := M_XRD // default to read command, can be changed based on state
  io.mem.req.bits.size    := log2Ceil(32).U                               // 32 -> max 65536
