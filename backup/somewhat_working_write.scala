...
class TestAcceleratorModule(outer: TestAccelerator)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
...
when (cmd.valid && doAccum && !reqValid && !busy) {
  reqAddr := rgPtr
  reqTag  := rgIdx
  reqData := 69.U // static value
  reqValid := true.B
...
cmd.ready := !reqValid && !busy
io.resp.valid := cmd.valid && !reqValid && !busy
io.resp.bits.rd := cmd.bits.inst.rd

io.busy := reqValid || busy
io.interrupt := false.B
// ...existing code...

    // when (io.mem.resp.valid && busy) {
    //   printf(cf"*ta*Memory response for tag ${memRespTag}.\n")
    //   busy := false.B // set busy to false when we receive a response
    // }

    // when (io.mem.req.fire) {
    //   printf(cf"*ta*Memory request for register ${rgIdx} at address ${rgPtr}.\n")
    //   // busy(rgIdx) := true.B
    //   busy := true.B // set busy to true when we send a request
    // }
    ...
    //   printf(cf"*ta*Init memory request function.\n")
    // }
    ...
    // // MEMORY REQUEST INTERFACE
    // io.mem.req.valid := cmd.valid && doAccum && !busy //&& !stallReg && !stallResp
    ...
    // io.mem.req.bits.signed := false.B
    // io.mem.req.bits.data := 69.U // we're not performing any stores...
    // io.mem.req.bits.phys := false.B
    // io.mem.req.bits.dprv := cmd.bits.status.dprv
    // io.mem.req.bits.dv := cmd.bits.status.dv
    // io.mem.req.bits.no_resp := false.B
